...
function hasNonMonFriSession(sessions) {
    return sessions.some(session => session.Day !== 'Mon' && session.Day !== 'Fri');
}

function compareByLongWeekend(timetableA, timetableB) {
    const hasNonMonFriA = hasNonMonFriSession(timetableA);
    const hasNonMonFriB = hasNonMonFriSession(timetableB);
    
    if (hasNonMonFriA && !hasNonMonFriB) return -1; 
    if (!hasNonMonFriA && hasNonMonFriB) return 1;  
    return 0; 
}
...
function parseCourseSchedules(data, userPreference) {
    ...
    data.forEach(course => {
        ...
        if (userPreference === '1') { // No early classes
            ...
            choices = choices.sort((a, b) => compareByLongWeekend(a.Sessions, b.Sessions));
        }
...
async function findConflictFreeSchedule(courseSchedules) {
    ...
    let userUnavailableTimes = []; // Step 2: Maintain a separate structure for unavailable times

    // Collecting user's unavailable times
    ...
    while (dayOff !== '-1') {
        ...
        userUnavailableTimes.push({ Day: dayOff, start: parseInt(startOff, 10), end: parseInt(endOff, 10) }); // Add to the list of unavailable times
        dayOff = await getUserInput('Enter the day you want off, enter -1 to stop: ');
    }

    
    // Check for conflicts including user's unavailable times
    Object.entries(updatedCourseSchedules).forEach(([courseCode, indexes]) => {
        indexes.some(indexInfo => {
            ...
            // Check each session against other sessions and user's unavailable times
            sessions.forEach(session => {
                userUnavailableTimes.forEach(unavailableTime => {
                    if (session.Day === unavailableTime.Day && !(session.end <= unavailableTime.start || session.start >= unavailableTime.end)) {
                        isConflict = true; // Mark as conflict if session overlaps with any unavailable time
                    }
                });
